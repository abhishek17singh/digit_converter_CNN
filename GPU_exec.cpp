...
// ReLU activation function on GPU (to process hidden layer values)
...
int main() {
    ...
    // Create a SYCL queue to run on the GPU (or CPU, depending on the device available)
    sycl::queue q(sycl::gpu_selector{});

    // Measure execution time using chrono
    auto start = std::chrono::high_resolution_clock::now();
    int predicted_digit = classify_digit(q, test_image);  // Perform inference on GPU
